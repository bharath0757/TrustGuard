"""Simulation service for controlled attack demonstrations.

Executes real security evaluations against TrustGuard's zero-trust engines
and persists real audit events to the database.
"""
...
from app.schemas.simulation import SimulationScenarioInfo, SimulationResponse
...
AVAILABLE_SCENARIOS: List[SimulationScenarioInfo] = [
    SimulationScenarioInfo(
        ...
        simulated_actor="Unauthenticated External Terminal / Unknown Client",
...
class SimulationService:
    ...
    async def run_simulation(
        ...
        if not scenario_info:
            # Fallback matching by scenario_id number or lowercase
        ...
        # Real security decision mapping according to TrustGuard core security rules
        ...
        # Persist REAL audit event to database
        event_details = {
        ...
        # Use exam_id for audit event linkage if provided, otherwise fall back to target paper string
        audit_exam_id = exam_id or target_paper

        audit_event = await AuditService.log_event(
            db=db,
            action=audit_action,
            exam_id=audit_exam_id,
            actor_id=simulated_actor,
            ip_address="127.0.0.1",
            details=event_details,
        )

        # Update ephemeral metrics if linked to a live exam
        if exam_id:
            from app.db.ephemeral import get_ephemeral_store
            ephemeral = get_ephemeral_store()
            await ephemeral.update_exam_session_metric(exam_id, "total_events")
            if actual_decision in ["BLOCKED", "INVALID AUTHORIZATION"]:
                await ephemeral.update_exam_session_metric(exam_id, "blocked_attempts")
                await ephemeral.update_exam_session_metric(exam_id, "unauthorized_attempts")
            if "INTEGRITY" in actual_decision:
                await ephemeral.update_exam_session_metric(exam_id, "integrity_violations")
